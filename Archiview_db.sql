-- 회원
DROP TABLE IF EXISTS acv_mbr;

-- 영화
DROP TABLE IF EXISTS acv_mov;

-- 태그
DROP TABLE IF EXISTS acv_tag;

-- 게시물
DROP TABLE IF EXISTS acv_post;

-- 태그_게시물
DROP TABLE IF EXISTS acv_tag_post;
...
-- 비밀번호 힌트 질문
DROP TABLE IF EXISTS acv_pw_hint_q;

-- 임시 테이블
DROP TABLE IF EXISTS Temporary3;

-- 저장한 회원 게시물
DROP TABLE IF EXISTS acv_save;

-- 영화 포스터
DROP TABLE IF EXISTS acv_pstr;

-- 회원 상태
DROP TABLE IF EXISTS acv_mbr_stat;

-- 장르
DROP TABLE IF EXISTS acv_gnr;

-- 국가
DROP TABLE IF EXISTS acv_ctr;

-- 영화 스틸컷
DROP TABLE IF EXISTS acv_stc;

-- 영화 장르
DROP TABLE IF EXISTS acv_gnr_mov;

-- 좋아요 이력
DROP TABLE IF EXISTS acv_like;

-- 팔로우 이력
DROP TABLE IF EXISTS acv_flw;

-- 관리자
DROP TABLE IF EXISTS acv_mg;

-- 사용자
DROP TABLE IF EXISTS acv_user;
...
CREATE TABLE acv_mbr (
		...
		pw_hint_a VARCHAR(50) NOT NULL -- 비밀번호 힌트 정답,
		rdt DATE DEFAULT 'now()' -- 회원 가입일,
...
CREATE TABLE acv_mov (
		...
		dir VARCHAR(50) NULL -- 영화 감독,
		eng_title VARCHAR(50) NULL -- 영화 영문명,
		runtime INTEGER NULL -- 상영시간,
		pdt INTEGER NULL -- 제작연도,
		odt DATE NULL -- 개봉일,
		actors MEDIUMTEXT NULL -- 출연,
		syn MEDIUMTEXT NULL -- 시놉시스,
		stat INTEGER NOT NULL -- 상태,
		rdt DATETIME DEFAULT 'now()' -- 등록일,
		cno INTEGER NULL -- 국가 번호
);
...
ALTER TABLE acv_mov
	MODIFY COLUMN mvno INTEGER NOT NULL AUTO_INCREMENT(1,1);
...
ALTER TABLE acv_tag
	MODIFY COLUMN tno INTEGER NOT NULL AUTO_INCREMENT(1,1);

-- 게시물
CREATE TABLE acv_post (
		pno INTEGER NOT NULL -- 게시물 번호,
		text MEDIUMTEXT NOT NULL -- 텍스트,
		rdt DATETIME DEFAULT 'now()' -- 등록일,
		stat INTEGER NOT NULL -- 상태,
		mvno INTEGER NOT NULL -- 영화 번호,
		stcno INTEGER NULL -- 스틸컷 번호,
		mno INTEGER NOT NULL -- 작성자 회원 번호
);
...
CREATE TABLE acv_pstr (
		...
		ps_url VARCHAR(255) NOT NULL -- 이미지 주소,
		mvno INTEGER NOT NULL -- 영화 번호
);
...
ALTER TABLE acv_pstr
	MODIFY COLUMN psno INTEGER NOT NULL AUTO_INCREMENT(1,1);
...
ALTER TABLE acv_mbr_stat
	MODIFY COLUMN stno INTEGER NOT NULL AUTO_INCREMENT(1,1);
...
ALTER TABLE acv_gnr
	MODIFY COLUMN gno INTEGER NOT NULL AUTO_INCREMENT(1,1);

-- 국가
CREATE TABLE acv_ctr (
		cno INTEGER NOT NULL -- 국가 번호,
		name VARCHAR(50) NOT NULL -- 국가명
);
...
CREATE TABLE acv_stc (
		...
		stc_url VARCHAR(255) NOT NULL -- 이미지 주소,
		mvno INTEGER NOT NULL -- 영화 번호
);
...
ALTER TABLE acv_stc
	MODIFY COLUMN stcno INTEGER NOT NULL AUTO_INCREMENT(1,1);
...
CREATE TABLE acv_like (
		pno INTEGER NOT NULL -- 좋아요한 게시물 번호,
		mno INTEGER NOT NULL -- 회원번호,
		ldt DATETIME DEFAULT 'now()' -- 좋아요 누른 일시
);
...
CREATE TABLE acv_flw (
		flwed_mbr INTEGER NOT NULL -- 팔로우된 회원번호,
		flwing_mbr INTEGER NOT NULL -- 팔로우한 회원번호,
		fdt DATETIME DEFAULT 'now()' -- 팔로우한 일시
);
...
ALTER TABLE acv_user
	MODIFY COLUMN uno INTEGER NOT NULL AUTO_INCREMENT(1,1);
