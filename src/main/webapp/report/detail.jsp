...
%>
	...
	<form action='update' method='post'>
		<p>
			신고번호 : <input type='text' name='no' value='<%=report.getNo()%>'readonly>
		</p>
		<p>
			신고 유형 : <%=getType(report.getReportedType())%></p>
		<p>
			신고자 : <%=report.getReportingMember().getNo()%></p>
		<p>
			신고일 : <%=report.getReportedDate()%></p>
		<p>
			신고 내용 : <%=report.getWhy()%></p>
      ...
      %>
		...
		<p>
			처리내용 : <textarea name='processingContent'><%=report.getProcessingContent()%></textarea>
		</p>
		<p>
			처리일 : <input type='date' name='processedDate' value='<%=report.getProcessedDate()%>'>
		</p>
		<p>
			<button>변경</button>
			<a href='delete?no=<%=report.getNo()%>'>[삭제]</a> <a href='list'>[목록]</a>
		</p>
	</form>
</body>
</html>
